---
version: 1
description: Capture, evolve, promote, or drop a test-strategy spike in `.llm/exploring/`. Drives the `llm-explore` skill.
...
2. **Dispatch by intent.** If `$ARGUMENTS` is:
   ...
   - An existing slug → ask: **evolve status** (idea → considering), **promote to campaign** (hand off to `/llm:plan` after preparing the body), or **drop** (`llm flow exploring/<slug> remove` after confirmation).
   - Empty → ask.
...
4. **Close out.** Run `llm doctor` and report. For a promote, surface the new `plans/<PLAN-ID>/` and remind the user to author the campaign frontmatter (test strategy + scope) via `/llm:plan` if not already done.

Hard rules:

- Spikes **never** flow to `archive/`. Only completed campaigns do. Drop = `llm flow exploring/<slug> remove` after explicit confirmation.
...
- The promote recipe hands off to `/llm:plan` to write campaign frontmatter — this command only stages the body and moves the dir.
